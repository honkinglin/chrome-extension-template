...
const createSuffix = require('./create-suffix')
const zipPlugin = require('./zip-plugin')

const SUFFIX = createSuffix();
const pluginName = `${crxConf.name}-${SUFFIX}`;
generatePem(crxConf.keyPath).then(info => {
  ...
  webpack(conf, (err, stats) => {
    ...
    zipPlugin(pluginName).then((size) => {
      console.log(chalk.cyan(`  Plugin zip complete. (${size}KB)\n`))
    })
  })
}).catch(err => {
